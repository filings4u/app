...
function renderSubscription(){
 ...
 const ss=$('#subscriptionStatus');if(ss){ss.outerHTML=`<select id="subscriptionStatus">${['trial','trialing','active','paused','past_due','cancelled'].map(v=>`<option value="${v}" ${v===String(s.status||'active')?'selected':''}>${esc(pretty(v))}</option>`).join('')}</select>`}
...
function renderTestingConfig(){
 const root=$('#managementContent'),defaults=detail.order_defaults||[],services=detail.orderable_services||[],labs=detail.lab_accounts||[],reporting=detail.lab_result_reporting||{};
 root.innerHTML=`<section class="admin-inline-editor"><div class="management-section-head"><div><h3>Order Defaults</h3><p>Manage values displayed on Employer testing orders.</p></div></div><form id="orderDefaultForm" class="saas-form"><div class="saas-form-grid"><label><span>Field Name</span><input name="field_name" required placeholder="Reason for Test"></label><label><span>Field Value</span><input name="field_value" required></label><label class="config-choice"><span>Show</span><input name="show" type="checkbox" checked></label><label class="config-choice"><span>Make Default</span><input name="make_default" type="checkbox"></label></div><div class="saas-actions"><button class="btn btn-orange">Save Default</button></div></form></section>${table(['Field','Value','Shown','Default'],defaults.map(x=>`<tr><td>${esc(x.field_name)}</td><td>${esc(x.field_value)}</td><td>${x.show?'Yes':'No'}</td><td>${x.make_default?'Yes':'No'}</td></tr>`).join(''),'No testing defaults configured.')}<section class="admin-inline-editor" style="margin-top:18px"><div class="management-section-head"><div><h3>Result Reporting Configuration</h3><p>Existing structured configuration is preserved as JSON so unsupported keys are not invented.</p></div></div><form id="resultReportingForm" class="saas-form"><label><span>Configuration JSON</span><textarea name="config_json" rows="7">${esc(JSON.stringify(reporting,null,2))}</textarea></label><div class="saas-actions"><button class="btn btn-orange">Save Result Reporting</button></div></form></section><section class="card" style="margin-top:18px"><div class="card-head"><div><h2>Orderable Services</h2></div><button id="saveOrderableServices" class="btn btn-outline" type="button">Save Services</button></div><div class="card-body">${table(['Service','Code','Orderable','Default'],services.map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(x.service_code)}</td><td><input type="checkbox" data-service-orderable="${esc(x.id)}" ${x.orderable?'checked':''}></td><td><input type="checkbox" data-service-default="${esc(x.id)}" ${x.is_default?'checked':''}></td></tr>`).join(''),'No Employer orderable services configured.')}</div></section><section class="card" style="margin-top:18px"><div class="card-head"><div><h2>Laboratory Accounts</h2></div><button id="saveLabAccounts" class="btn btn-outline" type="button">Save Lab Visibility</button></div><div class="card-body">${table(['Account','Code','Status','Hide On Order'],labs.map(x=>`<tr><td>${esc(x.name||'Lab Account')}</td><td>${esc(x.account_code||'—')}</td><td>${esc(pretty(x.status))}</td><td><input type="checkbox" data-lab-hide="${esc(x.id)}" ${x.hide_on_order?'checked':''}></td></tr>`).join(''),'No Employer laboratory accounts configured.')}</div></section>`;
 const df=root.querySelector('#orderDefaultForm');df.onsubmit=async ev=>{ev.preventDefault();const x=formData(df);x.show=df.elements.show.checked;x.make_default=df.elements.make_default.checked;try{await configApi({action:'save_order_defaults',employer_id:employerId,items:[x]});status('Order default saved.','success');await refresh()}catch(e){status(e.message,'error')}};
 const rf=root.querySelector('#resultReportingForm');rf.onsubmit=async ev=>{ev.preventDefault();try{const cfg=JSON.parse(rf.elements.config_json.value||'{}');await configApi({action:'save_lab_result_reporting',employer_id:employerId,config:cfg});status('Result reporting configuration saved.','success');await refresh()}catch(e){status(e.message,'error')}};
 root.querySelector('#saveOrderableServices')?.addEventListener('click',async()=>{try{await configApi({action:'save_orderable_services',employer_id:employerId,items:services.map(x=>({id:x.id,orderable:!!root.querySelector(`[data-service-orderable="${x.id}"]`)?.checked,is_default:!!root.querySelector(`[data-service-default="${x.id}"]`)?.checked}))});status('Orderable services saved.','success');await refresh()}catch(e){status(e.message,'error')}});
 root.querySelector('#saveLabAccounts')?.addEventListener('click',async()=>{try{await configApi({action:'save_lab_accounts',employer_id:employerId,items:labs.map(x=>({id:x.id,hide_on_order:!!root.querySelector(`[data-lab-hide="${x.id}"]`)?.checked}))});status('Laboratory visibility saved.','success');await refresh()}catch(e){status(e.message,'error')}});
}
