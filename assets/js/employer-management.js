...
export async function invokeEmployerAdmin(payload){
  const {data,error}=await supabase.functions.invoke((payload?.action==='list'||payload?.action==='detail')?'workforce-admin-employer-context':'workforce-admin-employer-actions',{body:payload});
